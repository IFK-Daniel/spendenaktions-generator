...
| `SESSION_SECRET` | HMAC-Signierschlüssel für das interne Session-Cookie (`api/_lib/sessionAuth.js`) | Interner Login (`/api/login`) **und** Zugriffsschutz für `/api/reserve-ifk-id` | Server | **Ja — kritisch** (wer diesen Wert kennt, kann gültige Sessions fälschen) | Keiner | `/api/login` antwortet mit 500 "Login ist derzeit nicht verfügbar" — bewusst fail-closed: **kein** Login möglich, solange kein Session-Cookie ausgestellt werden kann, statt einer unsicheren Notlösung ohne Session-Schutz |

**Insgesamt 12 Variablen** (das Redis-Paar zählt einfach, unabhängig davon, welches der beiden Namensschemata verwendet wird), alle ausschließlich serverseitig (keine `VITE_*`/`import.meta.env`-Variablen im gesamten Code gefunden — der Client kennt keine Secrets).
...
- **Genau ein gemeinsames Administrator-Konto** — keine Benutzerverwaltung, keine Rollen, keine Datenbank.
- **Seit `fix: protect ifk id reservation endpoint` (2026-09-05): serverseitig verifizierbare Session, nicht mehr nur clientseitiger Zustand.** Bei Erfolg setzt `/api/login` zusätzlich zu `{ ok: true }` ein signiertes Session-Cookie (`api/_lib/sessionAuth.js`):
  - **Cookie-Name:** `ifk_intern_session`.
  - **Inhalt:** ein zustandsloses, HMAC-SHA256-signiertes Token (`SESSION_SECRET`) mit ausschließlich einem Ablaufzeitstempel als Payload — **keine Zugangsdaten, keine Personendaten** im Cookie.
  - **Cookie-Attribute:** `HttpOnly` (kein Zugriff aus clientseitigem JavaScript, schützt gegen Diebstahl per XSS), `Secure` (nur über HTTPS), `SameSite=Strict`, `Path=/api` (wird nur an API-Endpunkte gesendet, an keine sonstigen Assets).
  - **Session-Lebensdauer:** 12 Stunden ab Ausstellung (`SESSION_TTL_MS` in `api/_lib/sessionAuth.js`), danach ist das Token unabhängig vom Cookie selbst ungültig (Ablaufzeitstempel ist Teil der signierten Payload, vom Client nicht manipulierbar).
  - **Bewusst kein Session-Store/keine neue Auth-Plattform:** die Gültigkeit steckt vollständig im signierten Token selbst — kein zusätzlicher Datenbank-Zugriff pro Request nötig, kein Redis-Verbrauch für Sessions.
  - `/api/logout` löscht das Cookie serverseitig (`Max-Age=0`) — da das Token zustandslos ist, kann ein bereits ausgestelltes, noch nicht abgelaufenes Token bei Logout nicht *invalidiert* werden (kein Blocklist-Mechanismus, bewusst nicht eingeführt — "keine neue große Auth-Plattform"); es kann nach einem Logout nur nicht mehr automatisch vom Browser mitgesendet werden. Praktisch beschränkt das Risiko auf: ein Angreifer müsste das Cookie vor dem Logout bereits kopiert haben, und selbst dann läuft es spätestens nach 12 Stunden ab.
- Der rein clientseitige "angemeldet"-Zustand in `window.sessionStorage` (`ifk_intern_authenticated`, `core/auth/authSession.js`, `src/intern/auth.js`) bleibt zusätzlich bestehen — er steuert ausschließlich die UI (Login-/App-Bereich sichtbar/versteckt), **nicht mehr die Autorisierung**: Diese liegt jetzt vollständig serverseitig beim Session-Cookie.
- **`POST /api/reserve-ifk-id` ist seit demselben Fix nur noch mit gültiger Session aufrufbar** (siehe Abschnitt "IFK-ID" und "Zugriffsschutz für `/api/reserve-ifk-id`" unten) — vorher war dieser schreibende, zustandsverändernde Endpunkt ohne jede Prüfung öffentlich erreichbar.
- Kleines **Login-Rate-Limit** als Defense-in-depth (`api/login.js`): max. 10 Anmeldeversuche pro IP innerhalb von 5 Minuten (Fixed-Window-Zähler über die ohnehin vorhandene Redis-Instanz, `redisIncrWithExpiry` in `api/_lib/upstashRedis.js` — keine neue Infrastruktur). Bewusst **fail open**: ist Redis nicht konfiguriert/erreichbar, wird gar nicht erst gezählt und der Login funktioniert normal — ein Ausfall des Rate-Limits darf niemals den Login blockieren, das schließt die vorher dokumentierte Lücke "kein Rate-Limiting" nur zusätzlich, ersetzt aber nicht die Zugangsdaten-Prüfung selbst.
  - **Redis-Key-Schema:** `ifk:loginrate:<IP-Adresse>` (IP aus `x-forwarded-for`, Fallback `req.socket.remoteAddress`), Value ein einfacher Zähler.
  - **Datenschutz/Lebensdauer der IP-Adresse:** Der Key erhält beim allerersten Anmeldeversuch aus dieser IP innerhalb des Fensters ein Ablaufdatum von exakt **300 Sekunden (5 Minuten)** (Redis `EXPIRE`). Nach Ablauf wird der Key automatisch von Redis gelöscht — die IP-Adresse wird **nicht dauerhaft gespeichert**, sondern ausschließlich für die Dauer des rollierenden 5-Minuten-Fensters als reiner Zählwert vorgehalten, danach ist jede Spur davon weg. Kein Logging der IP an anderer Stelle in diesem Zusammenhang.
  - **Nach Ablauf des Fensters:** Der Zähler beginnt bei der nächsten Anfrage aus derselben IP wieder bei 1 (neuer Key, neues 5-Minuten-Fenster) — legitime Nutzer:innen sind spätestens nach 5 Minuten wieder uneingeschränkt anmeldefähig, auch nach einem versehentlichen Fehlversuchs-Cluster.
- **Bekannte Sicherheitsgrenzen (weiterhin gültig):**
...
**Öffentlicher Generator** (`index.html`): kein Login, keine Authentifizierung — bewusst öffentlich zugänglich, unverändert durch diesen Fix.
...
- **SessionStorage:** Genau EIN Verwendungszweck im gesamten Code — der rein clientseitige UI-Anzeigezustand "angemeldet" (`ifk_intern_authenticated`, siehe Authentifizierungs-Abschnitt). Keine Personendaten darin; seit dem Sicherheitsfix (`fix: protect ifk id reservation endpoint`) steuert dieser Wert ausschließlich die UI-Sichtbarkeit, nicht mehr die tatsächliche Autorisierung — die liegt jetzt im serverseitigen, `HttpOnly`-Session-Cookie (dem Client-JavaScript also gerade NICHT zugänglich).
...
- **Vercel Storage/externe Datenbank:** **Seit 2026-09-05 eine Ausnahme:** Upstash Redis (siehe Abschnitt "Externer Dienst: Upstash Redis") — ausschließlich für IFK-ID-Reservierungs-Keys (`ifk:id:<ID>`) und die transienten Login-Rate-Limit-Zähler (`ifk:loginrate:<IP>`, automatisch nach 5 Minuten verfallend). Keine Personendaten in beiden Fällen. Keine sonstigen Datenbank-Abhängigkeiten in `package.json`.
...
| Übernahme einer bestehenden IFK-ID | Über das Formularfeld `ifk-id-input` (manuelle Eingabe) oder per Screenshot-Import (`handleApplyScreenshotFields()` in `src/intern/generator.js`); Validierung über `validateIfkId()` (Format-Check, auch bereits in der OCR-Extraktion selbst — `core/screenshot/buildExtractionFields.js` markiert eine formal ungültige Screenshot-IFK-ID nie als `recognized`, sie kann also nie automatisch übernommen werden). Eine so übernommene bestehende ID wird **nicht automatisch reserviert** — nur über "Neu generieren" erzeugte IDs durchlaufen `reserveIfkId()`. Für bereits andernorts (humbee) vergebene Alt-IDs siehe Alt-ID-Import unten. **Seit `fix: allow screenshot ifk id to override generated value` (2026-09-05):** Die IFK-ID unterliegt beim Screenshot-Import exakt derselben Überschreibe-Logik wie alle anderen Felder (Vorname, Nachname, E-Mail, Telefon, Bundesland, Region, Geschlecht) — zählt zu "vorhandene Formulardaten" (löst ggf. die Überschreiben-Bestätigung aus) und wird bei Bestätigung ausnahmslos ersetzt, auch wenn zuvor bereits eine (ggf. per "Neu generieren" erzeugte) ID im Feld stand. Keine Sonderbehandlung mehr zugunsten einer bereits vorhandenen ID. |
| Verworfene/ersetzte, zuvor generierte IDs (verwaiste Reservierungen) | **Bekannte, bewusst nicht automatisch behobene Nebenwirkung:** Eine per "Neu generieren" erzeugte ID wird sofort atomar in Redis reserviert (siehe "Kollisionsvermeidung" oben) — unabhängig davon, ob sie später tatsächlich verwendet wird. Wird sie danach manuell überschrieben oder (seit obigem Fix) durch eine Screenshot-ID ersetzt, **bleibt die ursprünglich generierte ID in Redis als reserviert stehen**, obwohl sie nie tatsächlich vergeben wurde — sie zählt ab dann dauerhaft als "verbraucht" im 32.768-ID-Namensraum. Dieses Verhalten bestand bereits **vor** diesem Fix (manuelles Überschreiben einer generierten ID hatte denselben Effekt) und wird durch die Korrektur des Screenshot-Imports nicht neu eingeführt, sondern lediglich um einen weiteren Auslöser (Screenshot-Override) ergänzt. **Bewusst keine automatische Freigabe gebaut:** Das Redis-Schema exponiert absichtlich kein `DEL`/Freigabe-Kommando über die API (nur `SET ... NX`, siehe Zugriffsschutz-Abschnitt) — eine automatische "gib die alte ID frei, wenn das Feld sich ändert"-Logik könnte im Zweifel eine ID freigeben, die zwischenzeitlich (z. B. in einem anderen Tab oder durch einen anderen Nutzer) bereits tatsächlich verwendet wurde, was riskanter wäre als eine handvoll ungenutzter, aber sicher reservierter IDs. Bei 32.768 möglichen Kombinationen und der aktuellen (kleinen) Nutzungszahl ist dieser Verbrauch praktisch vernachlässigbar; sollte er relevant werden, wäre ein sicherer nächster Schritt ein eigener, session-geschützter Freigabe-Endpunkt mit expliziter Bestätigung — aktuell nicht umgesetzt. |
...
## Sicherheitsfix: Zugriffsschutz für `/api/reserve-ifk-id`

**Gefundenes Problem (2026-09-05, unmittelbar nach Produktivsetzung der Redis-Reservierung):** `POST /api/reserve-ifk-id` war ohne jede Prüfung öffentlich aufrufbar — der interne Login (`api/login.js`) bestätigte eine erfolgreiche Anmeldung bis dahin ausschließlich mit `{ ok: true }`; der "angemeldet"-Zustand wurde ausschließlich clientseitig in `sessionStorage` gehalten (siehe vorherige Fassung dieses Audits). Der Server konnte einen Request also nicht von einem angemeldeten Browser unterscheiden. Da jede erfolgreiche Reservierung dauerhaften Zustand verändert (eine IFK-ID aus dem 32.768 Kombinationen umfassenden Namensraum wird endgültig entzogen), hätte ein externer Aufrufer ohne Login programmatisch massenhaft IDs reservieren und damit den verfügbaren Bestand erschöpfen bzw. manipulieren können — **kein personenbezogener Datenabfluss** (siehe Datenschutz-Abschnitt: der Endpunkt gibt und speichert ausschließlich die ID selbst), aber eine Verfügbarkeits-/Integritätslücke.

**Fix (Commit `fix: protect ifk id reservation endpoint`):**

- `/api/login` setzt bei erfolgreicher Anmeldung zusätzlich ein serverseitig signiertes, `HttpOnly`/`Secure`/`SameSite=Strict`-Session-Cookie (Details siehe Authentifizierungs-Abschnitt oben) — **kein Client-seitiges Secret**, kein im Bundle sichtbarer API-Key, kein einfacher auslesbarer Custom-Header.
- `/api/reserve-ifk-id` prüft dieses Cookie serverseitig (`hasValidSession()`, `api/_lib/sessionAuth.js`) als **allererste** Prüfung, vor jeder Body-/Formatverarbeitung — ohne gültige Session: `401`, **keine** Redis-Operation (weder lesend noch schreibend).
- `/api/logout` löscht das Cookie serverseitig.
- Der bestehende Alt-ID-Import-Weg (`scripts/import-ifk-ids.mjs`) ist von diesem Fix nicht betroffen — er nutzt weiterhin direkten, credential-basierten Redis-Zugriff, keinen HTTP-Endpunkt. Der für den initialen Import einmalig genutzte Umweg über den (damals ungeschützten) Endpunkt ist damit für künftige Importe geschlossen (siehe "Zugriffsweg" oben).
- Zusätzliches Rate-Limit auf `/api/login` (Defense-in-depth, siehe Authentifizierungs-Abschnitt) — schließt beiläufig auch die zuvor dokumentierte Lücke "kein Rate-Limiting/Brute-Force-Schutz im Login-Endpunkt".

**Nicht verändert:** ID-Format, die bereits reservierten 53 IFK-IDs, Flyer/Urkunden/PDF-Rendering/Mailversand/Rollenlogik, Redis-Schema (`ifk:id:<ID>` → `"1"`).
...
| Zugriffsweg | REST-API über die o. g. Umgebungsvariablen, ausschließlich serverseitig (`api/reserve-ifk-id.js`, `api/login.js` fürs Rate-Limit) — kein Client-seitiger Zugriff, kein direkter Redis-Zugriff aus dieser Session heraus (der REST-Token selbst wurde mir zu keinem Zeitpunkt mitgeteilt oder gelesen). **Wichtig:** Der initiale Alt-ID-Import (siehe unten) lief am 2026-09-05 einmalig über den damals noch ungeschützten `POST /api/reserve-ifk-id`-Endpunkt — dieser Weg ist seit `fix: protect ifk id reservation endpoint` **bewusst geschlossen** (der Endpunkt verlangt jetzt eine interne Session). Künftige administrative Importe laufen ausschließlich über `scripts/import-ifk-ids.mjs` mit direktem, credential-basiertem Redis-Zugriff — kein öffentlicher/Browser-Endpunkt dafür mehr offen. |
...
| Interner Generator | Fähigkeit, `MATERIAL_ADMIN_USERNAME`/`MATERIAL_ADMIN_PASSWORD` **und** `SESSION_SECRET` in Vercel zu ändern (= Vercel-Zugang, kein separates System) | Kein eigenes Login-/Recovery-System (siehe Authentifizierungs-Abschnitt) — Rotation läuft ausschließlich über den Vercel-Zugang. Ein Rotieren von `SESSION_SECRET` invalidiert augenblicklich **alle** bestehenden Sessions (jedes Token ist gegen den alten Schlüssel signiert und wird ab dem nächsten Deployment als ungültig erkannt) — nützlich als Notfall-"alle abmelden"-Maßnahme, macht aber auch nach einem versehentlichen Rotieren einen erneuten Login für alle Nutzer:innen nötig. |
