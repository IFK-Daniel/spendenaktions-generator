...
/**
 * Misst das reale Standard-Starter-Set (Vorgabe Abschnitt 11): Flyer
 * Druckerei Du, Flyer Home Du, Urkunde, PayPal-QR schwarz, GiroCode
 * schwarz, Anleitung — KEINE Sie-Varianten. Nutzt exakt denselben
 * Rendering-Code wie die Produktivumgebung (`buildFlyerVariantEntries`
 * mit `salutationVariants: ["du"]`, entspricht dem, was
 * `applyStarterSet()` in `src/intern/generator.js` setzt).
 *
...
files.push({ filename: "IFK_Daniel_Feigenbutz_Repraesentantenurkunde.pdf", content: Buffer.from(cert.bytes) });
...
const multipartPayloadBytes = (await new Response(formData).arrayBuffer()).byteLength;

const MAX_REQUEST_BYTES = 4_450_000;
const targetPayloadBytes = MAX_REQUEST_BYTES * 0.9;
console.log(`\nMultipart-Payload | ${multipartPayloadBytes} Byte (${(multipartPayloadBytes / 1024 / 1024).toFixed(2)} MB)`);
console.log(`MAX_REQUEST_BYTES | ${MAX_REQUEST_BYTES} Byte`);
console.log(`Anteil am Limit | ${((multipartPayloadBytes / MAX_REQUEST_BYTES) * 100).toFixed(1)}%`);
console.log(`Reserve | ${((1 - multipartPayloadBytes / MAX_REQUEST_BYTES) * 100).toFixed(1)}%`);
console.log(`Passt mit >=10% Reserve: ${multipartPayloadBytes <= targetPayloadBytes ? "JA" : "NEIN"}`);
  ...
  JSON.stringify(
    {
      ...
      percentOfLimit: (multipartPayloadBytes / MAX_REQUEST_BYTES) * 100,
      reservePercent: (1 - multipartPayloadBytes / MAX_REQUEST_BYTES) * 100,
      fitsWith10PercentReserve: multipartPayloadBytes <= targetPayloadBytes,
    },
    null,
    2
  )
